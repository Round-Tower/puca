...
Review: Kev + claude-opus-4-8, 2026-09-10 — added an optional MurphySig provenance
  footer (visible line + machine-readable Signed: comment) so every report carries its
  own signature, not just the source. Confidence now 0.8.
"""
...
def render_html(findings, *, engagement: str, target: str, generated: str,
                positives=None, method: str = "", tickets=None,
    ...
    sig_html = (f'<p class="murphysig"><b>MurphySig</b> {_esc(signature)} '
               f'<a href="https://murphysig.dev">murphysig.dev</a></p>'
               f'<!-- Signed: {_esc(signature)} -->') if signature else ''
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  footer .murphysig { color:var(--fg); margin:0 0 .8rem; }
  footer .murphysig b { color:var(--accent); }
...
<footer>$signature
<p>Generated by Púca 0.1.0 — Round&nbsp;Tower. White-hat, scope-gated assessment. Private report describing live systems; do not distribute.</p></footer>
</div>
</body></html>
""")
    return doc.substitute(
        ...
        positives=positives_html, method=method_html, signature=sig_html,
    )
